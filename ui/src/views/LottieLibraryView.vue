    ...
    <section class="library-layout">
      ...
      <section class="content-panel">
        <div class="content-heading">
          ...
          <div class="content-actions"><label class="page-size-control">每页<select v-model.number="pageSize" aria-label="每页显示数量"><option :value="12">12</option><option :value="24">24</option><option :value="48">48</option><option :value="96">96</option></select></label><button class="quiet-button" type="button" :disabled="loading" @click="load">刷新</button><button class="quiet-button" type="button" @click="toggleSelectionMode">{{ selectionMode ? '完成选择' : '批量管理' }}</button><button v-if="selectionMode && pagedAnimations.length" class="quiet-button" type="button" @click="toggleSelectVisible">{{ pagedAnimations.every((item) => selectedNames.includes(item.metadata.name)) ? '取消全选' : '全选当前页' }}</button></div>
        </div>
