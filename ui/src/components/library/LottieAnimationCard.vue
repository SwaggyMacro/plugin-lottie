...
const emit = defineEmits<{ configure: [Animation]; remove: [Animation]; select: [Animation]; pointerdown: [Animation, PointerEvent] }>()
...
<template>
  <article class="animation-card" :data-animation-name="props.animation.metadata.name" :class="{ disabled: props.animation.spec.enabled === false, selected: props.selectable && props.selected, sortable: props.sortable, dragging: props.dragging, 'drag-over': props.dragOver }" :role="props.selectable ? 'button' : undefined" :tabindex="props.selectable ? 0 : undefined" @click="handleCardClick" @keydown.enter.prevent="handleCardClick" @keydown.space.prevent="handleCardClick" @pointerdown="emit('pointerdown', props.animation, $event)">
